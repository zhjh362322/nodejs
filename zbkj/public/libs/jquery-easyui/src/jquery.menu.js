...
(function($){
	...
	function createItem(target, div, options){
		...
		if (!item.hasClass('menu-sep')){
			...
			if (itemOpts.iconCls){
				$('<div class="menu-icons"></div>').addClass(itemOpts.iconCls).appendTo(item);
			}
			if (itemOpts.id){
	...
	$.fn.menu.methods = {
		...
		/**
		 * set the menu icons class
		 * param: {
		 * 	target: DOM object, indicate the menu item
		 * 	iconCls: the menu item icons class
		 * }
		 */
		setIcon: function(jq, param){
			return jq.each(function(){
				...
				$(param.target).children('div.menu-icons').remove();
				if (param.iconCls){
					$('<div class="menu-icons"></div>').addClass(param.iconCls).appendTo(param.target);
				}
			});
		},
		/**
