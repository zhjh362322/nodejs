	...
	function updateTab(container, param){
		...
		if (param.type == 'all' || param.type == 'header'){
			...
			if (opts.header){
				...
				var s_icon = tab.find('span.tabs-icons');
				s_title.html(opts.title);
				s_icon.attr('class', 'tabs-icons');
				...
				if (opts.iconCls){
					s_title.addClass('tabs-with-icons');
					s_icon.addClass(opts.iconCls);
				} else {
					s_title.removeClass('tabs-with-icons');
				}
				if (opts.tools){
